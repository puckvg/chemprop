...
def run_first_k_fold(args: TrainArgs,
                   train_func: Callable[[TrainArgs, MoleculeDataset, Logger], Dict[str, List[float]]]
                   ) -> Tuple[float, float]:
    """
    Creates k-fold splits, but runs only on the first split.
    To allow hyperopt on first split only, with the split defined by k fold splits.
    ...
    for i, fold_num in enumerate(range(args.num_folds)):
        # CHANGED here to run only on first fold
        if i > 0:
            continue
    ...
    # Report results
    info(f'1-fold cross validation (single fold)')
